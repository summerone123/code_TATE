...
class Trainer(object):
	...
	def train(self):
		...
		for entity_episode, cluster_episode in self.train_environment.get_episodes(self.batch_counter):
			...
			# get the final reward from the environment, include entity_rewards, cluster_rewards,LLM_rewards
			entity_rewards = entity_episode.get_reward()
			cluster_rewards = cluster_episode.get_reward()
			LLM_rewards = LLM_evaluate(entity_episode)
			...
			LLM_rewards_torch = torch.tensor(LLM_rewards).to(self.device)
			...
			LLM_cum_discounted_reward = self.calc_cum_discounted_reward_credit(LLM_episode.credits,
																			 LLM_rewards_torch)  
			LLM_reinforce_loss = self.calc_reinforce_loss(LLM_all_losses, LLM_all_logits, LLM_cum_discounted_reward,
														current_decay, self.baseline_e)

			reinforce_loss = e_reinforce_loss + c_reinforce_loss + LLM_reinforce_loss

			self.baseline_e.update(torch.mean(e_cum_discounted_reward))
			self.baseline_c.update(torch.mean(c_cum_discounted_reward))
			self.baseline_c.update(torch.mean(LLM_cum_discounted_reward))
			...
			LLM_avg_reward = np.mean(LLM_rewards)
